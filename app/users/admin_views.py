...
class AdminUserViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def update_user(
        ...
        sync_results = None
        if any(
            k in request.data for k in ("plan_id", "is_active", "expire_date")
        ):
            sync_results = []
    ...
    def destroy_user(
        ...
        return Response(
            {"success": True, "message": f"用户 {username} 已删除"}
        )
    ...
    def post(self, request: Request) -> Response:
        ...
        is_valid = any(
            [
                ...
                magic.startswith(b"GIF87a")
                or magic.startswith(b"GIF89a"),  # GIF
                magic.startswith(b"RIFF") and magic[4:8] == b"WEB",  # WebP
            ]
        )
        if not is_valid:
            return Response(
                {"error": "文件格式验证失败，请上传有效图片"},
                status=400,
            )
