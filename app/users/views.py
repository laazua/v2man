...
class RegisterView(generics.CreateAPIView):
    ...
    def create(
        ...
        **kwargs: Any,
        ...
        # 生成并发送验证码
        email = user.email
        verify_code = f"{secrets.randbelow(10**6):06d}"
        ...
        logger.info(
            "用户注册: id=%s username=%s email=%s invite_code=%s",
            user.id,
            user.username,
            email,
            code_str or "无",
    ...
    def post(self, request: Request) -> Response:
        ...
        if user is not None:
            ...
            try:
                ...
                logger.error("验证码重发邮件失败: email=%s", email)
...
class PaymentCreateView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        logger.info(
            "创建支付订单: user_id=%s amount=%s out_trade_no=%s " "driver=%s",
            request.user.id,
            amount,
            out_trade_no,
    ...
    def post(self, request: Request) -> Response:
        ...
        if user is not None:
            ...
            try:
                ...
                logger.error("密码重置邮件发送失败: email=%s", email)
