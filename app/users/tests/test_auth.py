...
class UsernameOrEmailBackendTest(TestCase):
    ...
    def test_authenticate_nonexistent_user_returns_none(self):
        """用户不存在返回 None"""
        result = self.backend.authenticate(
            None, username='nobody', password='testpass123',
        )
    ...
    def test_authenticate_empty_username_returns_none(self):
        """空用户名返回 None"""
        result = self.backend.authenticate(
            None, username='', password='testpass123',
        )
        self.assertIsNone(result)
    ...
    def test_authenticate_empty_password_returns_none(self):
        """空密码返回 None"""
        result = self.backend.authenticate(
            None, username='testuser', password='',
        )
        self.assertIsNone(result)
...
class ProfileViewTest(TestCase):
    ...
    def test_profile_plan_name(self):
        ...
        plan = Plan.objects.create(
            name='白银套餐', price=5.00, traffic_limit=1024, duration_days=30,
        )
        self.user.plan = plan
        self.user.save()
        self._auth()
...
class RechargeViewTest(TestCase):
    ...
    def test_recharge_get_returns_history(self):
        """GET 返回充值历史列表"""
        Recharge.objects.create(user=self.user, amount=1000, status='completed')
...
class PasswordResetTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client = APIClient()
        self.user = User.objects.create_user(
            'resetuser', 'reset@example.com', 'oldpass123',
        )
    ...
    def test_password_reset_request_nonexistent_email_returns_400(self):
        """邮箱不存在返回 400"""
        resp = self.client.post('/api/auth/password-reset/', {
            'email': 'nobody@example.com',
        })
        self.assertEqual(resp.status_code, 400)
        self.assertIn('未注册', resp.json()['error'])
    ...
    def test_password_reset_confirm_short_password_returns_400(self):
        ...
        self.assertEqual(resp.status_code, 400)
        self.assertIn('至少8位', resp.json()['error'])
...
class PaymentCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client = APIClient()
        self.user = User.objects.create_user(
            'payuser', 'pay@example.com', 'testpass123',
        )
...
class PaymentNotifyViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            'notifyuser', 'notify@example.com', 'testpass123',
        )
        self.other_user = User.objects.create_user(
            'otheruser', 'other@example.com', 'testpass123',
        )
    ...
    def test_payment_notify_unauthenticated_returns_401(self):
        """未认证返回 401"""
        resp = self.client.post('/api/auth/payment/notify/', {
            'out_trade_no': 'PAY123',
        })
        self.assertEqual(resp.status_code, 401)
    ...
    def test_payment_notify_wrong_user_returns_403(self):
        ...
        PaymentOrder.objects.create(
            user=self.other_user, amount=1000,
    ...
    def test_payment_notify_order_already_paid_returns_400(self):
        ...
        PaymentOrder.objects.create(
            ...
            out_trade_no='PAY_DONE_001', status='paid',
        )
        secret = self._compute_secret()
        self._auth()
        resp = self.client.post('/api/auth/payment/notify/', {
...
class PaymentOrderListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client = APIClient()
        self.user = User.objects.create_user(
            'orderuser', 'order@example.com', 'testpass123',
        )
    ...
    def test_payment_orders_list(self):
        ...
        PaymentOrder.objects.create(
            user=self.user, amount=2000,
            out_trade_no='ORD_002', status='paid',
        )
        self._auth()
...
class ThrottleTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client = APIClient()
        self.user = User.objects.create_user(
            'throttleuser', 'throttle@example.com', 'testpass123',
        )
    ...
    def test_verification_code_throttle_returns_429(self):
        ...
        User.objects.create_user(
            'vcodeuser', 'vcode@example.com', 'testpass123',
            is_active=False,
        )
        self.client.post('/api/auth/verification-code/resend/', {
    ...
    def test_activate_throttle_returns_429(self):
        ...
        User.objects.create_user(
            'actuser', 'act@example.com', 'testpass123',
            is_active=False,
        )
        for _ in range(11):
            self.client.post('/api/auth/activate/', {
...
class SignalTest(TestCase):
    ...
    def test_creates_wallet_and_subscription_on_user_creation(self):
        ...
        user = User.objects.create_user(
            'signaluser', 'signal@example.com', 'testpass123',
        )
    ...
    def test_signal_only_on_creation(self):
        ...
        user = User.objects.create_user(
            'signaluser2', 'signal2@example.com', 'testpass123',
        )
