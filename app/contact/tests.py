...
class ContactMessageListCreateViewTests(TestCase):
    ...
    def test_get_contact_list_returns_user_messages(self) -> None:
        ...
        ContactMessage.objects.create(
            user=self.user, subject='我的工单', message='help',
        )
        ContactMessage.objects.create(
            user=self.other, subject='其他人工单', message='other',
        )
    ...
    def test_unauthenticated_returns_401(self) -> None:
        ...
        resp = self.client.post(
            '/api/contact/', {'message': 'test'}, format='json',
        )
        self.assertEqual(resp.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_user_only_sees_own_messages(self) -> None:
        ContactMessage.objects.create(
            user=self.user, subject='我的', message='a',
        )
        ContactMessage.objects.create(
            user=self.other, subject='他人的', message='b',
        )
        self.client.force_authenticate(self.user)
        resp = self.client.get('/api/contact/')
    ...
    def test_admin_only_sees_own_messages_in_user_view(self) -> None:
        ContactMessage.objects.create(
            user=self.admin, subject='管理员工单', message='x',
        )
        ContactMessage.objects.create(
            user=self.other, subject='普通用户', message='y',
        )
...
class ContactMessageReplyViewTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.ticket = ContactMessage.objects.create(
            user=self.user, subject='工单', message='问题',
        )
    ...
    def test_user_reply_own_ticket(self) -> None:
        ...
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            ContactMessage.objects.filter(parent=self.ticket).count(), 1,
        )
...
class ContactUnreadViewTests(TestCase):
    ...
    def test_unread_count_with_replied_tickets(self) -> None:
        self.client.force_authenticate(self.user)
        ContactMessage.objects.create(
            user=self.user, subject='已回复', message='a',
            status='replied',
        )
        ContactMessage.objects.create(
            user=self.user, subject='待回复', message='b',
            status='pending',
        )
        resp = self.client.get('/api/contact/unread-count/')
    ...
    def test_unread_count_with_after_filter(self) -> None:
        ...
        msg = ContactMessage.objects.create(
            user=self.user, subject='旧', message='x',
            status='replied', replied_at=timezone.now(),
        )
...
class AdminContactViewSetTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.ticket = ContactMessage.objects.create(
            user=self.user, subject='求助', message='help',
            status='pending',
        )

    def test_list_all_tickets(self) -> None:
        ContactMessage.objects.create(
            user=self.user, subject='第二个', message='more',
        )
        self.client.force_authenticate(self.admin)
    ...
    def test_pending_count(self) -> None:
        ContactMessage.objects.create(
            user=self.user, subject='另一个', message='x',
            status='pending',
        )
        ContactMessage.objects.create(
            user=self.user, subject='已回复', message='y',
            status='replied',
        )
        self.client.force_authenticate(self.admin)
    ...
    def test_toggle_visibility(self) -> None:
        ...
        reply = ContactMessage.objects.create(
            parent=self.ticket, user=self.user,
            message='回复', is_admin=True,
